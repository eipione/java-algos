package com.ephipi.java.threads;
...
public class BlockingThreadPoolExecutor
{
	...
	// We might be tempted to bind it with queueSize but that can be dangerous because
	// we release the semaphore in the last line *within* the execution of the thread
	// So, in a really close call, the task may be submitted before the thread becomes free
	// And, if queue is also full our execution will be rejected
	final LinkedBlockingQueue<Runnable> queue = new LinkedBlockingQueue<Runnable>();

	// Get permits = number of threads allowed + number of items allowed in queue
	...
	// This will block if all the permits are in use thereby stopping the generation of more work
	semaphore.acquire();
	...
	Runnable newTask = new Runnable()
	{
		...
		finally
		{
		    ...
		    // Ideally we would want to do this *after* the thread is returned to the pool
	...
	{
	    // Since we have already acquired a permit, in case our execution is rejected we want to release the permit. 
	    semaphore.release();
	}
	...
	// The output demonstrates that up to 15 are submitted out of which 10 start running and 5 are queued up
	for (int i = 0; i < 100; i++)
	{
